...
if (typ == IDENTITY) {
    ...
    yc =  gm_params[ref][4] * x +
         (gm_params[ref][5] - (1 << WARPEDMODEL_PREC_BITS)) * y +
          gm_params[ref][1]
    if (allow_high_precision_mv) {
      mv[0] = Round2(xc, WARPEDMODEL_PREC_BITS - 3)
      mv[1] = Round2(yc, WARPEDMODEL_PREC_BITS - 3)
    } else {
      mv[0] = Round2(xc, WARPEDMODEL_PREC_BITS - 2) * 2
      mv[1] = Round2(yc, WARPEDMODEL_PREC_BITS - 2) * 2
    }
}
~~~~~
...
The inputs to this process are:
  ...
  * a variable notOnBottom that is equal to 1 if the transform block is not on
...
The array aboveRow[ i ] for i = 0..w + h - 1 is derived as follows:
  ...
  * Otherwise, the following applies:
    ...
    * The variable aboveLimit is set equal to Min( maxX, x + ( notOnRight ? 2 * txw : txw ) - 1 ).
...
The array leftCol[ i ] for i = 0..w + h - 1 is derived as follows:
  ...
  * Otherwise, the following applies:
    ...
    * The variable leftLimit is set equal to Min( maxY, y + ( notOnBottom ? 2 * txh : txh ) - 1 ).
...
A 2D array named pred containing the intra predicted samples is constructed as follows:
  ...
  * Otherwise if mode is equal to SMOOTH_PRED, the following ordered steps
    ...
    2. The variable smoothPred is set as follows:
       ...
       ~~~~~ c
       smoothPred =   smWeights[ log2H - 2 ][ i ] * aboveRow[ j ] +
                   ( 256 - smWeights[ log2H - 2 ][ i ] ) * leftCol[ h - 1 ] +
                     smWeights[ log2W - 2 ][ j ] * leftCol[ i ] +
                   ( 256 - smWeights[ log2W - 2 ][ j ] ) * aboveRow[ w - 1 ]
       ~~~~~
...
The variable isInterIntra is set equal to RefFrame[ 1 ] == INTRA_FRAME.
...
If isInterIntra is equal to 1, the following ordered steps apply:
  ...
  4. The variable notOnRight is set equal to 0
  
  5. The variable notOnBottom is set equal to 0
  
  6. The variable log2W is set equal to log2( w )
  
  7. The variable log2H is set equal to log2( h )

  8. The intra prediction process specified in [section 8.6.1][] is invoked with plane, x, y, haveLeft, haveAbove, notOnRight, notOnBottom, mode, log2W, log2H as inputs.  (This will write intra predicted samples into CurrFrame.)
...
The variable gamma0 is set equal to Clip3( -32768, 32767, Round2( v * divFactor, divShift ) ).

The variable w is set equal to ( warpParams[ 3 ] * warpParams[ 4 ] ).

The variable delta0 is set equal to Clip3( -32768, 32767, warpParams[ 5 ] - Round2( w * divFactor, divShift ) - (1 << WARPEDMODEL_PREC_BITS) ).
...
~~~~~ c
alpha = Round2( alpha0, WARP_PARAM_REDUCE_BITS) << WARP_PARAM_REDUCE_BITS
beta = Round2( beta0, WARP_PARAM_REDUCE_BITS) << WARP_PARAM_REDUCE_BITS
gamma = Round2( gamma0, WARP_PARAM_REDUCE_BITS) << WARP_PARAM_REDUCE_BITS
delta = Round2( delta0, WARP_PARAM_REDUCE_BITS) << WARP_PARAM_REDUCE_BITS
~~~~~
...
for (i = 0; i < 2; i++) {
    for (j = 0; j < 2; j++) {
        A[i][j] = Clip3(LS_MAT_MIN, LS_MAT_MAX, Round2(A[i][j], downshift))
    }
    Bx[i] = Clip3(LS_MAT_MIN, LS_MAT_MAX, Round2(Bx[i], downshift))
    By[i] = Clip3(LS_MAT_MIN, LS_MAT_MAX, Round2(By[i], downshift))
}
~~~~~
