...
These processes are triggered at points defined by function calls to find_mv_list and find_mv_stack
...
This process is triggered by a function call to find_mv_list and constructs an array RefListMv containing 2 motion vector candidates.

The array RefListMv is first initialized to have both entries equal to ZeroMv[refList].
...
If NumMvFound is equal to 0 or candMv is not equal to RefListMv[ 0 ], then the following ordered steps apply:

  1. RefListMv[ NumMvFound ] is set equal to candMv
  2. NumMvFound is set equal to NumMvFound + 1
  
#### Find MV Stack Process

This process is trigger by a function call to find_mv_stack.
...
This process constructs an array RefStackMv containing motion vector candidates.  For single prediction, these motion vectors are used to
override the RefListMv motion vectors constructed by find_mv_list.  The process also prepares the value of the contexts used
...
2. The scan row process in [section 8.5.2.1][] is invoked with deltaRow equal to -1 and refList as inputs. 

3. The scan col process in [section 8.5.2.2][] is invoked with deltaCol equal to -1 and refList as inputs. 

4. The scan point process in [section 8.5.2.3][] is invoked with deltaRow equal to -1, deltaCol equal to bw, and refList as inputs. 
...
the temporal scan process in [section 8.5.2.4][] is invoked with refList as input (the temporal scan process affects ZeroMvContext and FoundPrevMatch).

8. If FoundPrevMatch is equal to 0, ZeroMvContext is set equal to 1.

6. The scan point process in [section 8.5.2.3][] is invoked with deltaRow equal to -1, deltaCol equal to -1, and refList as inputs. 

7. The scan row process in [section 8.5.2.1][] is invoked with deltaRow equal to -3 and refList as inputs. 

8. The scan col process in [section 8.5.2.2][] is invoked with deltaCol equal to -3 and refList as inputs. 

9. The scan row process in [section 8.5.2.1][] is invoked with deltaRow equal to -5 and refList as inputs. 

10. The scan col process in [section 8.5.2.2][] is invoked with deltaCol equal to -5 and refList as inputs. 

11. The scan col process in [section 8.5.2.2][] is invoked with deltaCol equal to -7 and refList as inputs.

12. The sorting process in [section 8.5.2.9][] is invoked with start equal to 0 and end equal to numNearest.

13. The sorting process in [section 8.5.2.9][] is invoked with start equal to numNearest and end equal to NumMvFound.

13. The context and clamping process in [section 8.5.2.10][] is invoked with refList, numNearest, and numNew as input.
...
where the call to add_ref_mv_candidate invokes the process in [section 8.5.2.5][].
...
the add reference motion vector process in [section 8.5.2.5][] is invoked with mvRow, mvCol, refList, len, and usePrev as inputs.
...
1. if PrevRefFrames[ mvRow ][ mvCol ][ candList ] is equal to RefFrame[ refList ], the search stack process in [section 8.5.2.6][] is invoked with mvRow, mvCol, len, candList, usePrev as inputs
...
1. if RefFrames[ mvRow ][ mvCol ][ candList ] is equal to RefFrame[ refList ], the search stack process in [section 8.5.2.6][] is invoked with mvRow, mvCol, len, candList, usePrev as inputs
...
the compound search stack process in [section 8.5.2.7][] is invoked with mvRow, mvCol, refList, len as inputs
...
The lower precision process specified in [section 8.5.2.8][] is invoked with candMv.
...
For i = 0..1, the lower precision process specified in [section 8.5.2.8][] is invoked with candMvs[ i ].
...
This process computes contexts to be used when decoding syntax elements, and then clamps the motion vectors in the stack.
...
The variable allZero is set as follows:
...
The following ordered steps apply:
  ...
  1. For i = 0..(n0-1), for j = 0..(n0-1), the following ordered steps apply:
      ...
      a. If i is equal to 0 and j is equal to 0, the variable q is set equal to get_dc_quant( plane ).
      
      b. Otherwise (i, j or both are not equal to 0), the variable q is set equal to get_ac_quant( plane ).
      
      c. If using_qmatrix is equal to 1 and SegQMLevel[ segment_id ] is less than 15, the variable q is set
         equal to Round2( q * quantizer_matrix[ SegQMLevel[ segment_id ] ][ plane > 0 ][ qm_offset[ txSz ] + i * n0 + j ], 4 ).
      
      d. Dequant[ i ][ j ] is set equal to ( Tokens[ i * n0 + j ] *
         q ) / dqDenom.
  ...
  3. CurrFrame[ plane ][ y + i ][ x + j ] is set equal to
 ...
 * The variable ref, specifying the reference frame type (INTRA_FRAME, LAST_FRAME, GOLDEN_FRAME or ALTREF_FRAME),
