...
The variable ref (specifying the reference frame) is set equal to RefFrames[ refList ].
...
has_overlappable_candidates( ) {
  if ( AvailU ) {
    ...
    for ( x4 = MiCol; x4 < Min( MiCols, MiCol + w4 ); x4 += 2 ) {
        if ( RefFrames[ x4 | 1 ][ MiRow - 1 ][ 0 ] > INTRA_FRAME )
            return 1
    }
  }
  if ( AvailL ) {
    ...
    for ( y4 = MiRow; y4 < Min( MiRows, MiRow + h4 ); y4 += 2 ) {
        if ( RefFrames[ MiCol - 1 ][ y4 | 1 ][ 0 ] > INTRA_FRAME )
            return 1
    }
  }
  return 0
...
the following ordered steps:
  ...
  1. The variable useWarp is derived as follows:
  ...
  1. If plane is equal to 0 and motion_mode is equal to LOCALWARP, the warp estimation process in [section 8.6.2.8][] is invoked.
  ...
  5. If useWarp is equal to 1, the block warp process in [section 8.6.2.5][] is invoked with
  ...
  5. If useWarp is equal to 0, the block inter prediction process in [section 8.6.2.4][] is invoked with
