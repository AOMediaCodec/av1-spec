...
for ( i = 0; i < BLOCK_SIZE_GROUPS; i++ )
    adapt_probs( intra_mode_tree, y_modeProbs[ i ], counts_intra_mode[ i ] )
...
The variable mode is specified by:
  ...
  * Otherwise, (plane is equal to 0), mode is set equal to y_mode.
...
The function get_qindex( ignoreDeltaQ ) returns the quantizer index for the current block and is specified by the following:
  ...
  * If ignoreDeltaQ is equal to 0 and delta_q_present is equal to 1, return CurrentQIndex.

  * Othewise, if seg_feature_active( SEG_LVL_ALT_Q ) is equal to 1 the following ordered steps apply:
...
Then, the loop filter is applied on all vertical boundaries followed by all horizontal boundaries as
follows:
...
block edge) is derived as follows:
  ...
  * If pass is equal to 0 and xP is an exact multiple
    of 4*num4x4BlocksWideLookup[ planeSize ], isBlockEdge is set equal to 1.

  * Otherwise, if pass is equal to 1 and yP is an exact multiple
    of 4*num4x4BlocksHighLookup[ planeSize ], isBlockEdge is set equal to 1.
...
The filter size process specified in [section 8.8.3][] is invoked with the inputs
...
The adaptive filter strength process specified in [section 8.8.4][] is invoked with
the inputs row and col, and the output assigned to the variables
lvl, limit, blimit, and thresh.
      
If lvl is equal to 0, the adaptive filter strength process specified in [section 8.8.4][] is invoked with
