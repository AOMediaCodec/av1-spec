...
the following ordered steps:
  ...
  1. The variable isGlobal is derived as follows:
  
     * If MiSize < BLOCK_8X8, isGlobal is set equal to 0.
    
     * Otherwise, if gm_type[ RefFrame[ refList ] ] <= TRANSLATION, isGlobal is set equal to 0.
    
     * Otherwise, if YMode == ZEROMV or YMode == ZERO_ZEROMV, isGlobal is set equal to 1.
    
     * Otherwise, isGlobal is set equal to 0.
  ...
  5. If isGlobal is equal to 1, the block warp process in [section 8.6.2.5][] is invoked with
  ...
  5. If isGlobal is equal to 0, the block inter prediction process in [section 8.6.2.4][] is invoked with
...
The array warpParams is set equal to a copy of gm_params[ RefFrame[ refList ] ].
