...
For each coded frame in turn the decoding process operates as follows:
  ...
  3. The CDEF process specified in [section 8.10][] is invoked.
  ...
  4. The output process as specified in [section 8.11][] is invoked.

  5. The reference frame update process as specified in [section 8.12][] is
     invoked.
...
in the inter block mode info syntax described in [section 6.15.19][].
...
The setup zero mv process specified in [section 8.5.1.1][] is invoked with the input refList and the output is assigned to ZeroMvs[ refList ].
...
The following ordered steps apply:
  ...
  1. The add sample process in [section 8.5.5.1][] is invoked with deltaRow equal to -1, and deltaCol equal to 0..(w4 - 1) (this adds candidates above the current block).
  
  2. The add sample process in [section 8.5.5.1][] is invoked with deltaCol equal to -1, and deltaRow equal to 0..(h4 - 1) (this adds candidates to the left of the current block).

  3. The add sample process in [section 8.5.5.1][] is invoked with deltaRow equal to -1, and deltaCol equal to -1 (this adds a candidate to the top left).
  
  4. The add sample process in [section 8.5.5.1][] is invoked with deltaRow equal to -1, and deltaCol equal to w4 (this adds a candidate to the top right).
  
  1. The sort sample process in [section 8.5.5.2][] is invoked.
...
midX and midY give the central position of the candidate block in units of pixels.
...
the following ordered steps:
  ...
  3. If plane is equal to 0 and motion_mode is equal to LOCALWARP, the warp estimation process in [section 8.6.2.8][] is invoked.

  4. The motion vector selection process in [section 8.6.2.1][] is invoked
     with plane, refList as inputs and the output being the
     motion vector mv.

  5. The motion vector clamping process in [section 8.6.2.2][] is invoked with
  ...
  7. The motion vector scaling process in [section 8.6.2.3][] is invoked with
  ...
  8. If useWarp is equal to 1, the block warp process in [section 8.6.2.5][] is invoked with
  ...
  9. If useWarp is equal to 0, the block inter prediction process in [section 8.6.2.4][] is invoked with
...
If isInterIntra is equal to 1, the following ordered steps apply:
  ...
  8. The intra prediction process specified in [section 8.6.1][] is invoked with plane, x, y, haveLeft, haveAbove, haveAboveRight, haveBelowLeft, mode, log2W, log2H as inputs.  (This will write intra predicted samples into CurrFrame.)
     
An array named Mask is prepared as follows:

  * If compound_type is equal to COMPOUND_WEDGE and plane is equal to 0, the wedge mask process in [section 8.6.2.11][] is invoked with w, h as inputs.
  
  * Otherwise if compound_type is equal to COMPOUND_INTRA, the inter intra mask process in [section 8.6.2.13][] is invoked with preds, plane, x, y, w, h as inputs.
  
  * Otherwise if compound_type is equal to COMPOUND_SEG and plane is equal to 0, the segment mask process in [section 8.6.2.12][] is invoked with preds, w, h as inputs.
...
follows:
  ...
  * Otherwise, the mask blend process in [section 8.6.2.14][] is invoked with preds, plane, x, y, w, h as inputs.
   
If motion_mode is equal to OBMC, the overlapped motion compensation in [section 8.6.2.9][] is invoked with plane, x, y, w, h as inputs.
...
The setup shear process specified in [section 8.6.2.6][] is invoked with warpParams as input, and the outputs are assigned to alpha, beta, gamma, and delta.
...
The resolve divisor process specified in [section 8.6.2.7][] is invoked with warpParams[ 2 ] as input, and the outputs assigned to divShift and divFactor.
...
The resolve divisor process specified in [section 8.6.2.7][] is invoked with det as input, and the outputs assigned to divShift and divFactor.
...
When the function predict_overlap is invoked, the following ordered steps apply to form the overlap prediction for a region of size predW by predH based on the candidate motion vector:
  ...
  5. The motion vector scaling process in [section 8.6.2.3][] is invoked with
     plane, refIdx, predX, predY, mv as inputs and the output being the
     initial location startX, startY, and the step sizes stepX, stepY.

  6. The block inter prediction process in [section 8.6.2.4][] is invoked with
  ...
  8. The blending process in [section 8.6.2.10][] is invoked with plane, predX, predY, predW, predH, pass, obmcPred, and mask as inputs.
...
The row transforms with i = 0..(h-1) are applied as follows:
  ...
  * Otherwise, if PlaneTxType is equal to one of DCT_DCT, ADST_DCT, FLIPADST_DCT or H_DCT,
    ...
    1. Invoke the inverse DCT permutation process as specified in [section 8.8.1.2][] with the input
  ...
  * Otherwise if PlaneTxType is equal to one of DCT_ADST, ADST_ADST, DCT_FLIPADST, FLIPADST_FLIPADST, 
    ...
    specified in [section 8.8.1.9][] with input variable n equal to log2W.
    
  * Otherwise, invoke the inverse identity transform process specified in [section 8.8.1.15][] with the
    input variable n equal to log2W.
  ...
  * Otherwise, if PlaneTxType is equal to one of DCT_DCT, DCT_ADST, DCT_FLIPADST or
    V_DCT, apply an inverse DCT as follows:
      ...
      1. Invoke the inverse DCT permutation process as specified in
         [section 8.8.1.2][] with the input variable n equal to log2H.
...
in [section 8.9.1][] is invoked with the variables plane, pass, row, and col as inputs.
...
When the cdef_block function is called, the CDEF block process specified in [section 8.10.1][] is invoked with r, c, and idx as inputs.
...
If filterY is equal to 1 or filterUV is equal to 1, the CDEF direction process specified in [section 8.10.2][] is invoked with r and c as inputs, and the outputs assigned to variables yDir and var.
...
If filterY is equal to 1, the following ordered steps apply:
 ...
 8. The CDEF filter process specified in [section 8.10.3][] is invoked with plane equal to 0, r, c, priStr, secStr, damping, and dir as input.
...
If filterUV is equal to 1, the following ordered steps apply:
 ...
 6. The CDEF filter process specified in [section 8.10.3][] is invoked with plane equal to 1, r, c, priStr, secStr, damping, and dir as input.
 
 7. The CDEF filter process specified in [section 8.10.3][] is invoked with plane equal to 2, r, c, priStr, secStr, damping, and dir as input.
...
**Note:** These variables are used by the is_inside function to determine which samples are available for use in filtering.
If loop_filter_across_tiles is equal to 1, the variables are set to indicate the whole frame is available.
If loop_filter_across_tiles is equal to 0, the variables are set according to the tile that includes the current block.
