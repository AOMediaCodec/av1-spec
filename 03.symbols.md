...
| `MV_CONTEXTS`                    | 4         | Number of contexts for decoding motion vectors including one for intra block copy
| `MV_INTRABC_CONTEXT`             | 3         | Motion vector context used for intra block copy
...
| `COMP_GROUP_IDX_CONTEXTS`        | 7         | Number of contexts for `comp_group_idx`
...
| `SIG_REF_DIFF_OFFSET_NUM`        | 3         | Maximum number of context samples to be used in determining the context index for `coeff_base` and `coeff_base_eob`.
...
| `EOB_COEF_CONTEXTS`              | 22        | Number of contexts for `is_eob` and `eob_extra`
...
| `REFMVS_LIMIT`                   | 1 \<\< 12 | Largest reference MV component that can be saved
