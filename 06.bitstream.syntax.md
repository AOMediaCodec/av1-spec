...
uncompressed_header( ) {
    ...
    if ( !FrameIsIntra ) {
        for( i = 0; i < REFS_PER_FRAME; i++ ) {
            ...
            if (frame_type == SWITCH_FRAME || !enable_order_hint ) {
...
Sgr_Params is a constant lookup table defined in [section 7.15.3][] and decode_signed_subexp_with_ref_bool
