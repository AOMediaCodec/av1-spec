...
read_tx_size( allowSelect ) {
    ...
    maxCodedTxSize = Max_Coded_Tx_Size[ MiSize ]
    if ( MiSize > BLOCK_4X4 ) {
        if ( allowSelect && TxMode == TX_MODE_SELECT ) {
            @@tx_size                                                          S
            TxSize = maxCodedTxSize - tx_size
            if ( TxSize > maxTxSize )
...
The Max_Coded_Tx_Size table specifies the maximum transform size which
can be encoded for each block size:

~~~~~ c
Max_Coded_Tx_Size[ BLOCK_SIZES ] = {
    TX_4X4, TX_8X8, TX_8X8, TX_8X8,
    TX_16X16, TX_16X16, TX_16X16, TX_32X32,
    TX_32X32, TX_32X32, TX_64X64, TX_64X64,
...
residual( ) {
    ...
    num4x4W = Num_4x4_Blocks_Wide[ MiSize ]
    num4x4H = Num_4x4_Blocks_High[ MiSize ]
    
    for ( blockY4 = 0; blockY < num4x4H; blockY += 16 ) {
        for ( blockX4 = 0; blockX < num4x4W; blockX += 16 ) {
            
            subBlockMiRow = (MiRow + blockY4) & sbMask
            subBlockMiCol = (MiCol + blockX4) & sbMask
            ...
            for ( plane = 0; plane < 1 + HasChroma * 2; plane++ ) {
                ...
                block4x4W = Min( num4x4W, 16 ) >> subX
                block4x4H = Min( num4x4H, 16 ) >> subY
    
                log2W = Min( MI_SIZE_LOG2 + Mi_Width_Log2[ planeSz ], 5 - subX )
                log2H = Min( MI_SIZE_LOG2 + Mi_Height_Log2[ planeSz ], 5 - subY )
                
                baseX = ((MiCol + blockX4) >> subX) * MI_SIZE
                baseY = ((MiRow + blockY4) >> subY) * MI_SIZE
                candRow = ((MiRow + blockY4) >> subY) << subY
                candCol = ((MiCol + blockX4) >> subX) << subX
                ...
                if ( IsInterIntra || IsCFL ) {
                    predict_intra( plane, baseX, baseY,
                                   ...
                                   BlockDecoded[ plane ]
                                               ...
                                               [ ( subBlockMiCol >> subX ) + block4x4W ],
                                   BlockDecoded[ plane ]
                                               [ ( subBlockMiRow >> subY ) + block4x4H ]
                ...
                if ( is_inter ) {
                    ...
                    predH = block4x4H << 2
                    someUseIntra = 0
                    for( r = 0; r < block4x4H; r++ )
                        for( c = 0; c < block4x4W; c++ )
...
is_inside determines whether a candidate position is accessible.
Moving across the top tile edge is allowed when
dependent_tiles is equal to 1 and allowed by the AllowDependentTileRow array, but
...
decode_lr( r, c, bSize ) {
    ...
    for (plane = 0; plane < NumPlanes; plane++) {
        ...
        unitColStart = ( relMiCol * ( MI_SIZE >> subX) + unitSize - 1 ) / unitSize
        unitColEnd = Min( unitCols, ( (relMiCol + w) * ( MI_SIZE >> subX) + unitSize - 1 ) / unitSize)
