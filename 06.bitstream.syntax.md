...
uncompressed_header( ) {
    ...
    if ( !FrameIsIntra ) {
        for( i = 0; i < REFS_PER_FRAME; i++ ) {
            OrderHints[ LAST_FRAME + i ] = RefOrderHint[ ref_frame_idx[ i ] ]
        }
    }
    if ( error_resilient_mode ) {
...
~~~~~
skip_mode_params( ) {
    if ( FrameIsIntra || ReferenceMode == SINGLE_REFERENCE ) {
        skipModeAllowed = 0
    } else {
        forwardIdx = -1
        backwardIdx = -1
...
frame_reference_mode( ) {
    ...
    if ( FrameIsIntra || ReferenceMode == COMPOUND_REFERENCE) {
...
coeffs( plane, startX, startY, txSz ) {
    ...
    if ( all_zero ) {
        ...
        if ( plane == 0 )
            transform_type( x4, y4, Tx_Size_Sqr[ txSz ] )
