...
still_picture equal to 0 specifies that the bitstream contains one or more coded frames.
...
**decoder_model_info_present_flag** specifies whether decoder model information is present in the bitstream.
decoder_model_info_present_flag must be 0 if still_picture equals 1.

**initial_display_delay_present_flag** specifies whether display model information is present in the bitstream or not.
initial_display_delay_present_flag must be 0 if still_picture equals 1.
...
It is a requirement of bitstream conformance that initial_display_delay_minus_1[ i ] is less than or equal to BUFFER_POOL_MAX_SIZE - 1.
...
**bitrate_scale** together with bitrate_minus_1 specifies the maximum smoothing buffer input bitrate.

**buffer_size_scale** together with buffer_size_minus_1 specifies the smoothing buffer size.
...
to decode frames of different sizes and dimensions present in the bitstream.
...
**bitrate_minus_1[ op ]** together with bitrate_scale specifies the maximum smoothing buffer input bitrate for the operating point op, in bits/s, as follows:

~~~~~ c
BitRate[ op ] = ( bitrate_minus_1[ op ] + 1 ) << ( 6 + bitrate_scale )
~~~~~

bitrate_minus_1[ op ] shall be in the range of 0 to ( (1 \<\< 32) - 2).

**buffer_size_minus_1[ op ]** together with buffer_size_scale specifies the smoothing buffer size for
operating point op, in bits, as follows:

~~~~~ c
BufferSize[ op ] = ( buffer_size_minus_1[ op ] + 1 ) << ( 4 + buffer_size_scale )
~~~~~

buffer_size_minus_1[ op ] shall be in the range of 0 to ( (1 \<\< 32) - 2).

**cbr_flag[ op ]** equal to 0 indicates that the decoder model associated with operating point op operates in variable bitrate (VBR) mode.
cbr_flag[ op ] equal to 1 indicates that the decoder model associated with operating point op operates in constant bitrate (CBR) mode.
...
**encoder_buffer_delay[ op ]** specifies the time between the first bit of the coded frame
with the frame_type equal to KEY_FRAME entering the encoder buffer and the time that it is subsequently transmitted.
encoder_buffer_delay is measured in units of 1/90000 seconds.
encoder_buffer_delay is used in combination with buffer_removal_time to specify the delivery time of the coded frames to the smoothing buffer.
It corresponds to the encoder buffer fullness at the time that the first bit is transmitted. 
...
buffer_removal_time_present_flag equal to 0 specifies that buffer_removal_time is not present in the bitstream.

**buffer_removal_time[ opNum ]** specifies the frame removal time in units of DecCT
clock ticks counted from the removal time of the last frame with frame_type equal to KEY_FRAME
...
**RefFrameId[ i ]** specifies the frame id for each reference buffer.
...
If FrameIsIntra is equal to 0 (indicating that this frame may use inter prediction), the requirements described in the frame size with refs semantics of [section 6.7.5] must also be satisfied.
...
only read on the first non-skipped block of a superblock.
...
It is a requirement of bitstream conformance that whenever assign_mv returns, the function is_dv_valid would return 1, where is_dv_valid is defined as:
