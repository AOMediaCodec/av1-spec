...
| obu_type | Name of obu_type       | Frame-related
| -------- | ----------------       | ------------
| 0        | Reserved               | -
| 1        | OBU_SEQUENCE_HEADER.   | 0
| 2        | OBU_TEMPORAL_DELIMITER | 0
| 3        | OBU_FRAME_HEADER       | 1
| 4        | OBU_TILE_GROUP         | 1
| 5        | OBU_METADATA           | 0
| 6        | OBU_FRAME              | 1
| 7        | OBU_REDUNDANT_FRAME_HEADER | 1
| 8        | OBU_TILE_LIST          | 0
| 9-14     | Reserved               | -
| 15       | OBU_PADDING            | 0
...
The column “Frame-related” indicates if the corresponding OBU type is considered to be frame-related (value 1), or not (value 0).

**obu_extension_flag** indicates if the optional obu_extension_header is present. 

obu_extension_flag should be set to 0 in temporal delimiter and sequence header OBUs. 
...
In frame-related OBUs, when temporal_id is not present it is inferred to be equal to 0.

**spatial_id** specifies the spatial level of the data contained in the OBU.
In frame-related OBUs, when spatial_id is not present it is inferred to be equal to 0.
...
If operating_point_idc[ op ] is not equal to 0 for any value of op from 0 to operating_points_cnt_minus_1, it is a requirement of bitstream conformance that obu_extension_flag is equal to 1 for all OBUs with obu_type equal to one of OBU_FRAME_HEADER, OBU_TILE_GROUP, OBU_FRAME, or OBU_REDUNDANT_FRAME_HEADER in the coded video sequence.
...
If scalability metadata is present it should be placed between the first sequence header and the first frame header of a coded video sequence. The scalability metadata OBU must not have an OBU extension header. The information present in a scalability metadata OBU applies to the entire coded video sequence in which it is contained, and only that sequence. Redundant copies of a scalability metadata OBU may occur in any temporal unit of a coded video sequence.
