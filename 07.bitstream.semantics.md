...
See [section 7.11][] for details of the frame update process.
...
When this function is invoked the following ordered steps apply:
  ...
  2. PrevMvs is set equal to SavedMvs[ prevFrame ].
  
  3. PrevPredMvs is set equal to SavedPredMvs[ prevFrame ].
  
  4. PrevRefFrames is set equal to SavedRefFrames[ prevFrame ].
  
  5. gm_params is set equal to SavedGmParams[ prevFrame ].
  
  6. The motion field estimation process in [section 7.4][] is invoked.
...
The loop filter process is described in [section 7.8][].
...
coefficient). The Dequantization process is specified in [section 7.6][].
...
process specified in [section 7.5.1][] is invoked.
...
process specified in [section 7.5.2][] is invoked.
...
process specified in [section 7.5.3][] is invoked.
...
the reconstruction process specified in [section 7.6.2][] is invoked.
...
the predict chroma from luma process specified in [section 7.5.4][] is invoked.
