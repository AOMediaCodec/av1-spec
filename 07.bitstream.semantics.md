...
that the value of showable_frame for the previous frame was equal to 1.  In this case (i.e. when the previous frame had frame_type equal to KEY_FRAME), it is also a requirement of bitstream conformance that the frame is output via the show_existing_frame mechanism at most once.

**Note:** This requirement also forbids storing a frame into multiple reference frames and then using show_existing_frame for each reference frame.
{:.alert .alert-info }
