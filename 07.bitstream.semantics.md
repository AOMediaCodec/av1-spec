...
**Note:** It is legal to have still_picture equal to 1 and reduced_still_picture_header equal to 0.
...
**Note:** It is legal for two operating points to have the same value of operating_point_idc.
This can be used to indicate that a bitstream satisfies multiple level/tier combinations.
{:.alert .alert-info }

**level[ i ]** specifies the level that the coded video sequence conforms to when operating point i is selected.

**Note:** It is recommended that encoders should select the lowest level that is satisfied by the operating point.
However, this is not a strict requirement of bitstream conformance
(because in some cases it is hard to know the lowest level at the start of encoding).
{:.alert .alert-info }
...
**bitrate_scale**, **buffer_size_scale**, **encoder_decoder_buffer_delay_length_minus_1**,
**num_units_in_decoding_tick**, **buffer_removal_delay_length_minus_1**, and **frame_presentation_delay_length_minus_1**
are syntax elements used by the decoder model.  They do not affect the decoding process.

#### Operating Parameters Info Semantics

**bitrate_minus_1**, **buffer_size_minus_1**, **cbr_flag**,
...
If still_picture is equal to 1, it is a requirement of bitstream conformance that frame_type is equal to KEY_FRAME and show_frame is equal to 1.
...
**Note:** It is legal for primary_ref_frame to be coded as PRIMARY_REF_NONE, this will cause default
...
When this function is invoked the following ordered steps apply:
  ...
  3. The function load_loop_filter_params( prevFrame ) specified in [section 7.20][] is invoked.
  
  4. The function load_segmentation_params( prevFrame ) specified in [section 7.20][] is invoked.
...
saved CDFs. This process is described in [section 7.6][].
