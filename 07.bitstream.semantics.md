...
Metadata OBUs may or may not have an OBU extension header. If there is no extension header, the metadata OBU’s scope is all operating points in the coded video sequence, starting from the point the metadata OBU appears in the bitstream. If there is an extension header, the metadata OBU’s scope is for the specific layer identified in the OBU extension header, starting from the point it appears in the bitstream.  Specific metadata OBUs types may have additional constraints on the presence and use of the OBU extension header. 
