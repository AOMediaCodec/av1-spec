...
obu_extension_flag must [should?] be set to 0 in temporal delimiter and sequence header OBUs. 
...
In OBUs with obu_type equal to one of OBU_FRAME_HEADER, OBU_TILE_GROUP, OBU_FRAME, or OBU_REDUNDANT_FRAME_HEADER, when temporal_id is not present it is inferred to be equal to 0.

**spatial_id** specifies the spatial level of the data contained in the OBU.
In OBUs with obu_type equal to one of OBU_FRAME_HEADER, OBU_TILE_GROUP, OBU_FRAME, or OBU_REDUNDANT_FRAME_HEADER, when spatial_id is not present it is inferred to be equal to 0.
