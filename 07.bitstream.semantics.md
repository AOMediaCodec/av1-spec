...
of bits within the trailing bits.
{:.alert .alert-info }
...
showable_frame equal to 0 specifies that this frame will not be displayed using the show_existing_frame mechanism.
...
Boolean decoder. The size does not include the bytes used for tile_size or syntax elements sent before tile_size.
For the last tile in the tile group, tile_size is computed instead of being read and includes the OBU trailing bits.
{:.alert .alert-info }
