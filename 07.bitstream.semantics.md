...
**dropped_obu** is a dummy syntax element that is read when the decoding process should
ignore an OBU because it is not contained in the selected operating point.
It has no affect on the decoding process.
...
**decode_to_display_rate_ratio**, **initial_display_delay**, and **extra_frame_buffers** define parameters of the decoder rate model
that correspond to this operating point, but do not affect the decoding process.
