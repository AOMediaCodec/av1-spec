...
**still_picture** equal to 1 specifies that the bitstream contains only one coded frame, which is a key frame with show_frame equal to 1.
...
**Note:** It is allowed for two operating points to have the same value of operating_point_idc.
...
**maxTileHeightMi** contains the maximum height (in units of 4x4 luma samples) that can be used for a tile.
