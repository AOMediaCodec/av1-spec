...
bitstream that are detailed in [section 7.4][].
...
When this function is invoked the following ordered steps apply:
  ...
  1. If primary_ref_frame is not equal to PRIMARY_REF_NONE, the variable prevFrame is set equal to ref_frame_idx[ 0 ].
  
  2. If primary_ref_frame is not equal to PRIMARY_REF_NONE, gm_params is set equal to SavedGmParams[ prevFrame ].
  
  3. If primary_ref_frame is not equal to PRIMARY_REF_NONE, segmentation_enabled is equal to 1, RefMiCols[ prevFrame ] is equal to MiCols, and
  ...
  4. If primary_ref_frame is not equal to PRIMARY_REF_NONE, the function load_loop_filter_params( prevFrame ) specified in [section 7.20][] is invoked.
  
  5. If primary_ref_frame is not equal to PRIMARY_REF_NONE, the function load_segmentation_params( prevFrame ) specified in [section 7.20][] is invoked.
  
  6. If can_use_previous is equal to 1, the motion field estimation process in [section 7.8][] is invoked.
