...
**operating_points_decoder_model_count_minus_1** plus 1 specifies the number of decoder model operating points present in the bitstream.

**decoder_model_operating_point_idc[ opNum ]** specifies the corresponding operating point for a set of operating parameters. 

**display_model_param_present_flag** specifies whether initial_display_delay_minus_1 is present in the bitstream.

**initial_display_delay_minus_1** is a syntax element used by the decoder model.  The value does not affect the decoding process.

**decoder_model_param_present_flag** specifies whether the operating parameters info is present in the bitstream.
...
**opPtIdc** is used to determine which layers this frame appears in.

If opPtIdc is not equal to zero, it is a requirement of bitstream conformance that obu_extension_flag is equal to 1.
