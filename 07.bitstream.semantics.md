...
**spatial_layers_cnt_minus1** indicates the number of spatial layers present in the video sequence minus one.

**spatial_layer_description_present_flag** indicates when set to 1 that the 
spatial_layer_ref_id is present for each of the (spatial_layers_cnt_minus1 + 1) layers, or that it is 
...
the (spatial_layers_cnt_minus1 + 1) layers, or that it they are not present when set to 0.
...
if ( equal_picture_interval ) {
  ...
  ticksPerPicture = num_ticks_per_picture_minus1 + 1
}
