...
adjusted based on the observed counts. This process is described in [section 8.4.4][].

**average_cdf** is a function call that indicates that the
CDF arrays are set equal to the average of the CDFs at the end of each tile. This process is described in [section 8.4.5][].
...
is invoked the following takes place:
  ...
  * The cumulative distribution function arrays are reset to default values as follows:
    ...
    * InterFilterCdf is set to a copy of default_interp_filter_cdf
    
    * MvJointCdf is set to a copy of default_mv_joint_cdf
    
    * MvClassCdf is set to a copy of default_mv_class_cdf
    
    * MvFrCdf is set to a copy of default_mv_fr_cdf
    
    * MvClass0FrCdf is set to a copy of default_mv_class0_fr_cdf
...
#### Delta Quantizer Parameters Semantics

**delta_q_present** specifies whether delta quantizer values are present in the bitstream.

**delta_q_res** specifies the left shift which should be applied to decoded delta quantizer values.
...
The size does not include the bytes used for tile_group_header, trailing_bits, and tile_size_minus1.
...
However, LeftPartitionContext[ i ] are set to 0 for i = 0..MaxSb64Rows*8-1 because
...
However, AbovePartitionContext[ i ] are set to 0 for i = 0..MaxSb64Cols*8-1
...
#### Delta Quantizer Index Semantics

**ReadDeltaQ** specifies whether the current block is the first one in the current superblock. Delta
quantizer values are only read on the first block of a superblock.

**delta_q_abs** specifies the absolute value of the delta quantizer value being decoded.
If delta_q_abs is equal to DELTA_Q_SMALL, the value is encoded using the delta_q_rem_bits /
delta_q_abs_bits path (see below).

**delta_q_rem_bits** and **delta_q_abs_bits** encode the absolute value of the delta quantizer
value being decoded, where the absolute value of the delta quantizer value is of the form:

~~~~~ c
(1 << delta_q_rem_bits) + delta_q_abs_bits + 1
~~~~~

**delta_q_sign_bit** equal to 0 indicates that the delta quantizer value is positive; delta_q_sign_bit
equal to 1 indicates that the delta quantizer value is negative.
...
**intra_mode** and **uv_mode** specify the direction of intra
...
obtained. An offset is added to inter_mode to compute the y_mode as follows:
...
process specified in [section 8.5.3][] is invoked.
