...
**seq_choose_screen_content_tools** equal to 1 indicates that the allow_screen_content_tools syntax element
...
**disable_update_cdf** equal to 1 indicates that the end of frame CDF update is disabled;
disable_update_cdf equal to 0 indicates that the end of frame CDF update is enabled.
...
coefficient). The Dequantization process is specified in [section 7.9][].
...
**Note:** ForwardFrame and BackwardFrame are set to the closest forward and backward frames (as measured by frame_offset values).
If no backward frame is found, then the second closest forward frame is used.
(Forward prediction is when we use a frame for reference that is considered to be output before the current frame,
backward prediction is when we use a frame that has not yet been output.)
{:.alert .alert-info }
...
block being decoded.  It first read from the stream, and then postprocessed
...
**Note:** COMPOUND_AVERAGE, COMPOUND_INTRA, and COMPOUND_DISTANCE cannot be directly signalled with the compound_type
...
process specified in [section 7.8.1][] is invoked.
...
process specified in [section 7.8.2][] is invoked.
...
process specified in [section 7.8.3][] is invoked.
...
the reconstruction process specified in [section 7.9.2][] is invoked.
...
the predict chroma from luma process specified in [section 7.8.4][] is invoked.
