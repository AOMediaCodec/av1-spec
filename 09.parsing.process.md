...
[Section 9.3.1][] specifies how a CDF array is chosen for the syntax element.  The variable cdf is set equal to a reference to this CDF array.
...
where abovemode and leftmode are the intra modes used for the blocks immediately above and to the left of this
...
**y_mode**: The cdf for y_mode is given by TileYModeCdf[ ctx ] where the variable ctx is computed as size_group_lookup[ MiSize ].
...
**has_palette_y**: the cdf is given by TilePaletteYModeCdf[ MiSize - BLOCK_8X8 ][ ctx ] where ctx is
computed as follows:
...
**has_palette_uv**: the cdf is given by TilePaletteUVModeCdf[ MiSize - BLOCK_8X8 ][ ctx ] where ctx is
computed as follows:
