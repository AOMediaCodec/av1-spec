...
~~~~ c
rate = 3 + ( cdf[ N ] > 15 ) + ( cdf[ N ] > 31 ) + Min( floor( log2( N ) ), 2 )
tmp = 0
for (i = 0; i < N - 1; i++) {
...
~~~~~ c
    ...
    aboveHasCoeffs = AvailU && !Skips[ MiRow - 1 ][ MiCol ]
    leftHasCoeffs = AvailL && !Skips[ MiRow ][ MiCol - 1 ]
    
    if ( !aboveHasCoeffs && !leftHasCoeffs )
        ctx = 1
    else if ( !aboveHasCoeffs )
        ctx = ( 2 * Tx_Height[ TxSizes[ MiRow ][ MiCol - 1 ] ] ) > maxTxHeight
    else if ( !leftHasCoeffs )
        ctx = ( 2 * Tx_Width[ TxSizes[ MiRow - 1 ][ MiCol ] ] ) > maxTxWidth
    else
        ctx = ( 2 * ( Tx_Width[ TxSizes[ MiRow - 1 ][ MiCol ] ] +
                      Tx_Height[ TxSizes[ MiRow ][ MiCol - 1 ] ] ) ) > 
              ( maxTxWidth + maxTxHeight )
~~~~~

The cdf to return is given by:

  * TileTx64x64Cdf[ ctx ] if maxTxDepth is equal to TX_64X64.
  
  * TileTx32x32Cdf[ ctx ] if maxTxDepth is equal to TX_32X32.

  * TileTx16x16Cdf[ ctx ] if maxTxDepth is equal to TX_16X16.
...
**segment_id**: The cdf is given by TileSegmentId[ ctx ] where ctx is computed by:
...
**use_filter_intra**: the cdf is given by TileFilterIntraCdf[ TxSize ].
...
if ( AvailU && AvailL ) {
    ...
    if ( LeftIntra ) {
        ...
        if ( !LeftSingle )
            ctx = 4 * (!check_last(LeftRefFrame[ RefFrameSignBias[ BWDREF_FRAME ] ]))
...
if ( AvailU && AvailL ) {
    ...
    if ( LeftIntra ) {
        ...
        if ( !LeftSingle )
            ctx = 4 * (!check_golden(LeftRefFrame[ RefFrameSignBias[ BWDREF_FRAME ] ]))
...
~~~~~ c
fwd = Abs( OrderHints[ RefFrame[ 0 ] ] - OrderHint )
