...
get_coeff_base_ctx( txSz, plane, blockX, blockY, pos, c, isEob ) {
    ...
    for ( idx = 0; idx < SIG_REF_OFFSET_NUM; idx++ ) {
        ...
        if (refRow >= 0 &&
            ...
            refRow < height &&
            refCol < width ) {
            mag += Min( Abs( Dequant[ refCol ][ refRow ] ), 3 )
        }
    }
...
~~~~~ c
ctx = get_coeff_base_ctx(txSz, plane, startX, startY, scan[c], c, 1) - SIG_COEF_CONTEXTS + SIG_COEF_CONTEXTS_EOB
~~~~~
...
for ( idx = 0; idx < 3; idx++ ) {
    ...
    mag += Min( Dequant[ refCol ][ refRow ], COEFF_BASE_RANGE + NUM_BASE_LEVELS + 1 )
}
