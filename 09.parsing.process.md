...
An array cdf of length 2 is constructed as follows:
~~~~
cdf[ 0 ] = ( ( p << 15 ) + 256 – p ) >> 8
cdf[ 1 ] = 1 << 15
~~~~
...
The input to this process is an array cdf of length N which specifies the cumulative distribution for a symbol with N possible values.
...
The variable bits is set to 15 – floor( log2( BoolRange + 1 ) ).
This represents the number of new bits to be read from the bitstream.

**Note:** bits may be equal to 0, in which case the following four updates have no effect.
