...
equal to 8 whenever this process is invoked.
...
also used to make sure that frames do not end with a superframe marker by
ensuring all frames end with at least one zero byte. It is legal for frames to
end with more than one byte of padding.
{:.alert .alert-info }

The final CDF values are accumulated for each of the CDF arrays as follows (in
this code the left and right hand sides of each line are arrays of the same
size, the += operator is applied to each element of the arrays in turn):

~~~~~ c
SumYModeCdf += TileYModeCdf
SumUVModeCdf += TileUVModeCdf
SumPartitionCdf += TilePartitionCdf
SumSegmentIdCdf += TileSegmentIdCdf
SumSegmentIdPredictedCdf += TileSegmentIdPredictedCdf
SumTx8x8Cdf += TileTx8x8Cdf
SumTx16x16Cdf += TileTx16x16Cdf
SumTx32x32Cdf += TileTx32x32Cdf
SumInterpFilterCdf += TileInterpFilterCdf
SumMotionModeCdf += TileMotionModeCdf
SumNewMvCdf += TileNewMvCdf
SumZeroMvCdf += TileZeroMvCdf
SumRefMvCdf += TileRefMvCdf
SumCompoundModeCdf += TileCompoundModeCdf
SumDrlModeCdf += TileDrlModeCdf
SumIsInterCdf += TileIsInterCdf
SumCompModeCdf += TileCompModeCdf
SumSkipCdf += TileSkipCdf
SumCompRefCdf += TileCompRefCdf
SumCompBwdRefCdf += TileCompBwdRefCdf
SumSingleRefCdf += TileSingleRefCdf
SumMvJointCdf += TileMvJointCdf
SumMvClassCdf += TileMvClassCdf
SumMvClass0BitCdf += TileMvClass0BitCdf
SumMvFrCdf += TileMvFrCdf
SumMvClass0FrCdf += TileMvClass0FrCdf
SumMvClass0HpCdf += TileMvClass0HpCdf
SumMvBitCdf += TileMvBitCdf
SumMvHpCdf += TileMvHpCdf
SumTailTokenCdf += TileTailTokenCdf
SumAcHeadTokenCdf += TileAcHeadTokenCdf
SumDcHeadTokenCdf += TileDcHeadTokenCdf
SumPaletteYModeCdf += TilePaletteYModeCdf
SumPaletteUVModeCdf += TilePaletteUVModeCdf
...
SumIntraTxTypeSet1Cdf += TileIntraTxTypeSet1Cdf
SumIntraTxTypeSet2Cdf += TileIntraTxTypeSet2Cdf
SumInterTxTypeSet1Cdf += TileInterTxTypeSet1Cdf
SumInterTxTypeSet2Cdf += TileInterTxTypeSet2Cdf
SumInterTxTypeSet3Cdf += TileInterTxTypeSet3Cdf
SumUseObmcCdf += TileUseObmcCdf
SumInterIntraCdf += TileInterIntraCdf
SumCompRefTypeCdf += TileCompRefTypeCdf
SumCflSignCdf += TileCflSignCdf
SumUniCompRefCdf += TileUniCompRefCdf
SumWedgeInterIntraCdf += TileWedgeInterIntraCdf
SumCompoundTypeCdf += TileCompoundTypeCdf
SumInterIntraModeCdf += TileInterIntraModeCdf
SumCflAlphaCdf += TileCflAlphaCdf
~~~~~
...
[Section 9.3.1][] specifies how a CDF array is chosen for the syntax element.
...
variable ctx is computed as size_group_lookup[ MiSize ].

**uv_mode**: The cdf for uv_mode is given by TileUVModeCdf[ ctx ] where the
variable ctx is set equal to YMode.

**partition**: The cdf for partition is given by TilePartitionCdf[ ctx ] where
the variable ctx is computed as follows:
...
if ( c == 0 ) {
    ...
    maxX4 = MiCols >> sx
    maxY4 = MiRows >> sy
    w4 = tx_size_wide[ txSz ] >> 2
    h4 = tx_size_high[ txSz ] >> 2
    x4 = startX >> 2
    y4 = startY >> 2
    above = 0
    left = 0
...
| set | cdf |
|:---:|:---:|
| 1   | TileIntraTxTypeSet1Cdf[ tx_size_sqr_map[ txSz ] ][ YMode ]
| 2   | TileIntraTxTypeSet2Cdf[ tx_size_sqr_map[ txSz ] ][ YMode ]
{:.table }
...
| set | cdf |
|:---:|:---:|
| 1   | TileInterTxTypeSet1Cdf[ tx_size_sqr_map[ txSz ] ]
| 2   | TileInterTxTypeSet2Cdf[ tx_size_sqr_map[ txSz ] ]
| 3   | TileInterTxTypeSet3Cdf[ tx_size_sqr_map[ txSz ] ]
{:.table }
...
**cfl_alpha_u**: The cdf is given by TileCflAlphaCdf[ cfl_alpha_signs - 2 ].

**cfl_alpha_v**: The cdf is given by TileCflAlphaCdf[ ctx ] where ctx is computed as follows:

~~~~~ c
signU = ( ( cfl_alpha_signs + 1 ) * 11 ) >> 5
signV = ( cfl_alpha_signs + 1 ) - 3 * signU
ctx = signV * 3 + signU - 3
~~~~~
