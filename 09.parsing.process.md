...
decoder" and the function init_bool( sz ), exit_bool( readPadding ), and read_bool( p )
...
This process is invoked when the function exit_bool( readPadding ) is called from the syntax
structure.
...
If readPadding is equal to 1, the padding syntax element is read using the f(BoolMaxBits) parsing process.
...
The range and value are renormalized by the following ordered steps:
  ...
  1. The variable bits is set to 15 – FloorLog2( BoolRange ).
