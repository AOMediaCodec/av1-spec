...
uv_mode : the tree is intra_mode_tree:
...
kf_uv_mode_probs[ y_mode ][ node ].

**intra_mode**: the probability is given by y_mode_probs[ ctx ][ node ] where
ctx is computed by:
...
**sub_intra_mode**: the probability is given by y_mode_probs[ ctx ][ node ]
where ctx is set equal to 0.

**uv_mode**: the probability is given by uv_modeProbs[ ctx ][ node ] where ctx is
set equal to y_mode.
