...
decoder" and the functions init_symbol( sz ), exit_symbol( readPadding ), read_symbol( cdf ), and read_bool( p )
...
the range 0 to 255) to use for the decode of a binary syntax element.

The output of this process is the value of the decoded syntax element.
...
This process is invoked when the function exit_symbol( readPadding ) is called from the syntax
structure.
...
If readPadding is equal to 0 (this happens for the last tile in a tile group), no reading of padding
is performed at this level of the syntax.  The remamining bits will be read by trailing_bits at the end
of the OBU.
...
The range and value are renormalized by the following ordered steps:
  ...
  4. The variable SymbolValue is set to
     newData ^ ( ( ( SymbolValue + 1 ) \<\< bits ) - 1 ).
...
**uv_mode**: The cdf for uv_mode is derived as follows:
    ...
    * If Max( Block_Width[ MiSize ], Block_Height[ MiSize ] ) <= 32, the cdf is given by
...
~~~~~ c
    ...
    if ( AvailU && IsInters[ MiCol ][ MiRow - 1 ] ) {
        aboveW = Block_Size_Wide[ MiSizes[ MiCol ][ MiRow - 1 ] ]
    } else if ( AvailU ) {
    ...
    if ( AvailL && IsInters[ MiCol - 1 ][ MiRow ] ) {
        leftH = Block_Size_High[ MiSizes[ MiCol - 1 ][ MiRow ] ]
    } else if ( AvailL ) {
...
if (maxTxSz >= TX_8X8) {
    ...
    ctx = (txSzSqrUp != maxTxSz && maxTxSz > TX_8X8) +
...
~~~~~ c
ctx = (RefMvContext >> 1) * COMP_NEWMV_CTXS
ctx = ctx + Min(NewMvContext, COMP_NEWMV_CTXS - 1)
~~~~~
...
**wedge_index**: The cdf is given by TileWedgeIndexCdf.
